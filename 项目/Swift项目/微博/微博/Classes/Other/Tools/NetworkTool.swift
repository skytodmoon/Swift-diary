...
protocol NetworkToolProtocol {
    ...
    /// 搜索
    static func loadSearchResult(keyword: String, offset: Int, completionHandler:@escaping (_ weitoutiao: [TouTiao]) -> ())
    /// 获取首页不同分类的新闻内容(和视频内容使用一个接口)
    static func loadHomeCategoryNewsFeed(category: String, completionHandler:@escaping (_ nowTime: TimeInterval,_ newsTopics: [TouTiao])->())
    /// 获取一般新闻详情数据
    ...
    /// 获取新闻详情评论
    static func loadNewsDetailComments(offset: Int, weitoutiao: TouTiao, completionHandler:@escaping (_ comments: [NewsDetailImageComment])->())
    /// 获取新闻详情相关新闻
    static func loadNewsDetailRelateNews(fromCategory: String, weitoutiao: TouTiao, completionHandler:@escaping (_ relateNews: [TouTiao], _ labels: [NewsDetailLabel], _ userLike: UserLike?, _ appInfo: NewsDetailAPPInfo?, _ filter_wrods: [WTTFilterWord]) -> ())
    /// 解析视频的真实链接
    ...
    /// 悟空问答
    static func loadQuestionAnswerList(topicTitle: TopicTitle, weitoutiao: TouTiao, completionHandler:@escaping (_ questionAnswer: QuestionAnswer)->())
...
class NetworkTool: NetworkToolProtocol {
    ...
    /// 搜索
    class func loadSearchResult(keyword: String, offset: Int, completionHandler:@escaping (_ weitoutiao: [TouTiao]) -> ()) {
    ...
    /// 获取首页不同分类的新闻内容(和视频内容使用一个接口)
    class func loadHomeCategoryNewsFeed(category: String, completionHandler:@escaping (_ nowTime: TimeInterval,_ newsTopics: [TouTiao])->()) {
                ...
                for data in dataJSONs {
                    if let content = data["content"].string {
                        ...
                        do {
                            ...
                            let topic = TouTiao(dict: dict as! [String : AnyObject])
                            topics.append(topic)
                            print(dict)
                            print("-----------------------------")
                        } catch {
    ...
    /// 获取新闻详情评论
    class func loadNewsDetailComments(offset: Int, weitoutiao: TouTiao, completionHandler:@escaping (_ comments: [NewsDetailImageComment])->()) {
    ...
    /// 获取新闻详情相关新闻
    class func loadNewsDetailRelateNews(fromCategory: String, weitoutiao: TouTiao, completionHandler:@escaping (_ relateNews: [TouTiao], _ labels: [NewsDetailLabel], _ userLike: UserLike?, _ appInfo: NewsDetailAPPInfo?, _ filter_wrods: [WTTFilterWord]) -> ()) {
                ...
                if let data = json["data"].dictionary {
                    ...
                    if let relatedVideoToutiao = data["related_video_toutiao"] {
                        for dict in relatedVideoToutiao.arrayObject! {
                            let news = TouTiao(dict: dict as! [String: AnyObject])
                        ...
                        if ordered_info.count > 0 { // 说明 ordered_info 有数据
                            for orderInfo in ordered_info.arrayObject! { // 遍历，根据 name 来判断
                                ...
                                if name == "labels" { // 新闻相关类别,数组
                                    ...
                                    if let orders = ordered["data"] as? [AnyObject] {
                                        for dict in orders {
                                            let relatenews = TouTiao(dict: dict as! [String: AnyObject])
    ...
    /// 悟空问答
    class func loadQuestionAnswerList(topicTitle: TopicTitle, weitoutiao: TouTiao, completionHandler:@escaping (_ questionAnswer: QuestionAnswer)->()) {
