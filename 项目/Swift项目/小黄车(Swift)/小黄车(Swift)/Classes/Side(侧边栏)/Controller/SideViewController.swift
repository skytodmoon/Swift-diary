...
class SideViewController: UIViewController {
    ...
    fileprivate lazy var tableView: UITableView = {
        ...
        img.frame = CGRect(x: 0, y: 1, width: AppScreenW*0.82, height: 110)
        let footerView = UIView(frame: CGRect(origin: .zero, size: CGSize(width: AppScreenW*0.82, height: 111)))
    ...
    override func viewDidLayoutSubviews() {
        ...
        self.view.mg_width = AppScreenW*0.82
    }
}
...
extension SideViewController {
    ...
    @objc fileprivate func userViewClick() {
        AppNotificationCenter.post(name: NSNotification.Name(AppSideUserViewClickNoti), object: nil, userInfo: nil)
    }
    @objc fileprivate func footerViewClick() {
        AppNotificationCenter.post(name: NSNotification.Name(AppSideFooterViewClickClickNoti), object: nil, userInfo: nil)
    }
...
extension SideViewController: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        AppNotificationCenter.post(name: NSNotification.Name(AppSideTableViewCellClickNoti), object: nil, userInfo: ["row": indexPath.row])
    }
