...
class TopView: UIView,UITextFieldDelegate,APNumberPadDelegate {
    ...
    fileprivate func setUpUI() {
        ...
        inputTextField.snp.makeConstraints { (make) in
            make.top.equalTo(chargingLabel.snp.bottom).offset(AppGloabalMargin)
            make.left.equalToSuperview().offset(30)
            make.right.equalTo(sureBtn.snp.left).offset(-AppGloabalMargin/2)
        ...
        descripLabel.snp.makeConstraints { (make) in
            make.top.equalTo(inputTextField.snp.bottom).offset(AppGloabalMargin)
            make.centerX.equalToSuperview()
        }
