...
class SearchViewController: BaseViewController {
    ...
    private lazy var historyTableView: UITableView = {
        ...
        tw.delegate = self
        tw.dataSource = self
        tw.register(headerFooterViewType: SearchTHead.self)
        tw.register(cellType: BaseTableViewCell.self)
        tw.register(headerFooterViewType: SearchTFoot.self)
        return tw
    }()
    ...
    lazy var searchTableView: UITableView = {
        ...
        sw.delegate = self
        sw.dataSource = self
        sw.register(headerFooterViewType: SearchTHead.self)
    ...
    lazy var resultTableView: UITableView = {
        ...
        rw.delegate = self
        rw.dataSource = self
        rw.register(cellType: ComicTCell.self)
        return rw
    }()
    ...
    private func searchRelative(_ text: String) {
        if text.count > 0 {
            ...
            currentRequest = ApiProvider.request(Api.searchRelative(inputText: text), model: [SearchItemModel].self) { (returnData) in
...
extension SearchViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        if tableView == historyTableView {
            let head = tableView.dequeueReusableHeaderFooterView(SearchTHead.self)
    ...
    func tableView(_ tableView: UITableView, viewForFooterInSection section: Int) -> UIView? {
        if tableView == historyTableView && section == 1 {
            let foot = tableView.dequeueReusableHeaderFooterView(SearchTFoot.self)
            foot?.data = hotItems ?? []
            foot?.didSelectIndexClosure{ [weak self] (index, model) in
                let vc = ComicViewController(comicid: model.comic_id)
