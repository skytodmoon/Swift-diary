...
class CateListViewController: BaseViewController {
    ...
    private lazy var collectionView: UICollectionView = {
        ...
        cw.register(cellType: RankCCell.self)
        cw.register(cellType: TopCCell.self)
    ...
    private func loadData() {
        ApiLoadingProvider.request(Api.cateList, model: CateListModel.self) { (returnData) in
...
extension CateListViewController: UICollectionViewDelegateFlowLayout, UICollectionViewDataSource{
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        if indexPath.section == 0 {
            let cell = collectionView.dequeueReusableCell(for: indexPath, cellType: TopCCell.self)
            cell.model = topList[indexPath.row]
            return cell
        } else {
            let cell = collectionView.dequeueReusableCell(for: indexPath, cellType: RankCCell.self)
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        if indexPath.section == 0 {
            ...
            for tab in model.extra?.tabList ?? [] {
                ...
                vcs.append(ComicListViewController(argCon: tab.argCon,
                                                    argName: tab.argName,
                                                    argValue: tab.argValue))
            }
            let vc = PageViewController(titles: titles, vcs: vcs, pageStyle: .topTabBar)
