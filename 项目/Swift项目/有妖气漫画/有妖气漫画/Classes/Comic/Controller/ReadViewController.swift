...
class ReadViewController: BaseViewController {
    ...
    private lazy var collectionView: UICollectionView = {
        ...
        cw.delegate = self
        cw.dataSource = self
        cw.register(cellType: ReadCCell.self)
        cw.Head = RefreshAutoHeader {
    ...
    func loadData(with index: Int, isPreious: Bool, needClear: Bool, finished: ((_ finished: Bool) -> Void)? = nil) {
        ...
        if index <= -1 {
            ...
            ApiLoadingProvider.request(Api.chapter(chapter_id: chapterId), model: ChapterModel.self) { (returnData) in
                
                self.collectionView.Head.endRefreshing()
                self.collectionView.Foot.endRefreshing()
...
extension ReadViewController:UICollectionViewDelegateFlowLayout, UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(for: indexPath, cellType: ReadCCell.self)
