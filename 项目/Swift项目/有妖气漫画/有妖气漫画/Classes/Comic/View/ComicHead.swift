...
class ComicHeadCCell: BaseCollectionViewCell {
...
class ComicHead: UIView {
    ...
    private lazy var thmemView: UICollectionView = {
        ...
        tw.register(cellType: ComicHeadCCell.self)
        return tw
    }()
...
extension ComicHead: UICollectionViewDataSource {
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        return themes?.count ?? 0
    }
    
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(for: indexPath, cellType: ComicHeadCCell.self)
