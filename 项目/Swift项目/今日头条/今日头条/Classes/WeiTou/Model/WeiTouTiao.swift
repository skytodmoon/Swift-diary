...
class WeiTouTiao {
    ...
    var relateNewsCellHeight: CGFloat? {
        ...
        return (title!.boundingRect(with: size, options: .usesLineFragmentOrigin, attributes: [NSAttributedStringKey.font: UIFont.systemFont(ofSize: 15)], context: nil).size.height) + 30
    }
    ...
    var newDetailTitleHeight: CGFloat? {
        ...
        return title?.boundingRect(with: size, options: .usesLineFragmentOrigin, attributes: [NSAttributedStringKey.font: UIFont.systemFont(ofSize: 18)], context: nil).size.height
    }
