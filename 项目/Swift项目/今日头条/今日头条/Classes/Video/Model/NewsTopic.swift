        ...
        if imageLists == nil || imageLists?.count == 0 {
            ...
            if middle_image?.height != nil {
                ...
                if video_detail_info?.video_id != nil || (largeImageLists?.count)! > 0 {
                    ...
                    titleH = (title?.boundingRect(with: size, options: .usesFontLeading, attributes: [NSAttributedStringKey.font: UIFont.systemFont(ofSize: 17)], context: nil).size.height)!
                    ...
                    let size = CGSize(width: titleW, height: CGFloat(MAXFLOAT))
                    titleH = (title?.boundingRect(with: size, options: .usesFontLeading, attributes: [NSAttributedStringKey.font: UIFont.systemFont(ofSize: 17)], context: nil
                        ).size.height)!
