...
class AddTagViewController: UIViewController {
    ...
    private func setupNav() {
        ...
        navigationItem.leftBarButtonItem = PublishNavigationController.itemButton(title: "返回", self, #selector(cancel)).0
        let rightBarButtonWithButton = PublishNavigationController.itemButton(title: "完成", self, #selector(post))
    ...
    func setupView() {
        ...
        let btn1 = PublishTagButton.init(true)
        btn1.setTitle("搞笑", for: .normal)
        let btn2 = PublishTagButton.init(true)
    ...
    private func setTagButtonsFrame(_ buttons: [PublishTagButton]) -> CGFloat {
        ...
        for i in 0..<buttons.count  {
            ...
            var width = button.titleLabel!.text!.width(CGSize(width: 1000.0, height: 40.0), [NSAttributedStringKey(rawValue: NSAttributedStringKey.font.rawValue): button.titleLabel?.font ?? UIFont.systemFont(ofSize: 15.0)])
            ...
            if i == buttons.count - 1 {
                ...
                let previousButtonMaxX = previousButton.frame.maxX
                let placehoderTextW = textField.placeholder!.width(CGSize(width: 1000.0, height: 40.0), [NSAttributedStringKey(rawValue: NSAttributedStringKey.font.rawValue): button.titleLabel!.font])
                let textW = textField.text!.width(CGSize(width: 1000.0, height: 40.0), [NSAttributedStringKey(rawValue: NSAttributedStringKey.font.rawValue): button.titleLabel!.font])
    ...
    fileprivate func addButton() {
        let btn = PublishTagButton.init(true)
