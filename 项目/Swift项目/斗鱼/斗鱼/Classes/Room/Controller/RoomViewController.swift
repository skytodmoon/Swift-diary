...
extension RoomViewController {
    ...
    private func setupInfo(){
        ...
        print("------> + \(String(describing: anchor?.pic74))")
...
extension RoomViewController {
    ...
    fileprivate func loadAnchorLiveAddress() {
        ...
        NetWorkTools.requestData1(.get, urlString: URLString, parameters: parameters) { (result) in
            ...
            NetWorkTools.requestData1(.get, urlString: rURL, parameters: nil, finishedCallback: { (result) in
...
extension RoomViewController : SocketDelegate {
    ...
    func socket(_ socket: Socket, joinRoom user: UserInfo) {
         ...
         */
        let auttributedMutabkeStr =  AttributedStringGenerator.generateJoinOrLeaveRoom(user.name, true)
        // 将富文本发送给服务器
    ...
    func socket(_ socket: Socket, leaveRoom user: UserInfo) {
        ...
        let leaveRoomMAttrStr = AttributedStringGenerator.generateJoinOrLeaveRoom(user.name, false)
        chatContentView.insertMessage(leaveRoomMAttrStr)
        
    }
    
    // 发送聊天信息
    func socket(_ socket: Socket, chatMsg: ChatMessage) {
        ...
        let chatMsgMAttr = AttributedStringGenerator.generateTextMessage(chatMsg.user.name, chatMsg.text)
    ...
    func socket(_ socket: Socket, giftMsg: GiftMessage) {
         ...
         */
        let sendGiftMAttrMsg = AttributedStringGenerator.generateGiftMessge(giftMsg.user.name, giftMsg.giftname, giftMsg.giftUrl)
        // 7. 将内容显示到chatContentView 上面
        chatContentView.insertMessage(sendGiftMAttrMsg)
        
        // 8. 从服务器取出礼物模型,然后以动画的形式展示到用户面前,并将礼物送出
        let giftMsgModel = GiftMessageModel(senderName: giftMsg.user.name, senderURL: "icon2", giftName: giftMsg.giftname, giftURL: giftMsg.giftUrl)
