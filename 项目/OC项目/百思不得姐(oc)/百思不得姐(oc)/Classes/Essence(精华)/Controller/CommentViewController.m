...
static NSString *const CommentURL = @"http://api.budejie.com/api/api_open.php?a=dataList&appname=bs0315&asid=4D9488FE-E59B-41A2-9323-AC3934759456&c=comment&client=iphone&data_id=19114235&device=ios%20device&from=ios&hot=1&jbk=0&mac=&market=&openudid=f1e1e75d11ec5e8a96503b30220f196e37759455&page=1&per=50&udid=&ver=4.2";
...
//保存帖子的top_cmt
@property (nonatomic,strong) TopCommentItem *saved_top_cmt;
...
- (void)setBasic{
    ...
    //注册Cell
    [self.tableView registerNib:[UINib nibWithNibName:NSStringFromClass([CommentCell class]) bundle:nil] forCellReuseIdentifier:ID];
...
- (void)setupHeader{
    ...
    //添加Cell
    WholeCell *wholeCell = [WholeCell WholeCell];
    wholeCell.textItems = self.items;
    wholeCell.sy_size = CGSizeMake(ScreenW, self.items.cellHeight);
...
- (void)loadNewsComment
{
    ...
    //拼接URL
    NSRange range = [CommentURL rangeOfString:@"_id="];
    NSString *preUrl = [CommentURL substringToIndex:range.location + 4];
    ...
    [self.manager GET:url parameters:parameters progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        ...
        //最热评论模型
        self.hotComment = [TopCommentItem mj_objectArrayWithKeyValuesArray:responseObject[@"hot"]];
        
        //最新评论模型
        self.latestComment = [TopCommentItem mj_objectArrayWithKeyValuesArray:responseObject[@"data"]];
...
- (void)loadMoreComments
{
    ...
    TopCommentItem *cmt = [self.latestComment lastObject];
    ...
    [self.manager GET:url parameters:parameters progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        ...
        NSArray *newComment = [TopCommentItem mj_objectArrayWithKeyValuesArray:responseObject[@"data"]];
...
- (void)keyboardWillChangeFrame:(NSNotification *)note
{
    ...
    self.commentConstraint.constant = ScreenH - frame.origin.y;
...
- (TopCommentItem *)commentsInIndexPath:(NSIndexPath *)indexPath
{
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    if (header == nil) {
        ...
        header.contentView.backgroundColor = CommonBgColor;
