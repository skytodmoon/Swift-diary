...
//帖子中间图片内容
@property (nonatomic,weak) AllPictureView *pictureView;

//帖子中间音频的内容
@property (nonatomic,weak) SoundsView *soundsView;
...
@end
@implementation WholeCell

//图片Cell懒加载
- (AllPictureView *)pictureView
{
    if (_pictureView == nil) {
        AllPictureView *pictureView = [AllPictureView pictureView];
...
{
    if (_soundsView == nil) {
        SoundsView *soundsView = [SoundsView audioView];
...
//    //设置字体
//    [attrString addAttribute:NSFontAttributeName value:[UIFont stemFontOfSize:14] range:NSMakeRange(0, textItems.text.length)];
//    
//    //得到自定义行间距的UILabel的高度
//    
