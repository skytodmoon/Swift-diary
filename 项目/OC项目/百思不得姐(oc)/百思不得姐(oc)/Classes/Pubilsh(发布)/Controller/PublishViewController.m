...
- (void)viewDidLoad {
    ...
    CGFloat buttonStartY = (ScreenH - 2 * buttonH) * 0.6;
    ...
    for (int i = 0; i < images.count; i++) {
        QuickLoginButton *button = [[QuickLoginButton alloc] init];
    ...
    placardView.center = CGPointMake(ScreenW * 0.5, -(placardView.sy_size.width));
    [self.view addSubview:placardView];
    //标语动画(底下关于标语的动画方法注释同按钮注释一样)
    POPSpringAnimation *anim = [POPSpringAnimation animationWithPropertyNamed:kPOPViewCenter];
    CGFloat centerX = ScreenW * 0.5;
    CGFloat centenEndY = ScreenH * 0.2;
    CGFloat centerBeginY = centenEndY - ScreenH;
...
- (void)cancelWithCompletionBlock:(void (^)())completionBlock
{
    ...
    for (int i = beginIndex ; i < self.view.subviews.count; i++) {
        ...
        CGFloat centenY = subView.sy_centerY + ScreenH;
        /*
         //动画节奏(一开始很慢后面很快)
