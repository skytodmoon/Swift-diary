...
+(BaseTabBarViewController *)sharedController{
    @synchronized(self){
        if(controller == nil){
            ...
            NSArray *vcs = @[[[HomeViewController alloc]init],[[FlashViewController alloc]init],[[ShoppingViewController alloc]init],[[MyViewController alloc]init]];
            NSArray *images = @[@"v2_home",@"v2_order",@"shopCart",@"v2_my"];
            NSArray *selectedImages = @[@"v2_home_r",@"v2_order_r",@"shopCart_r",@"v2_my_r"];
            [controller addChildVc:vcs titles:titles images:images selectedImages:selectedImages tabBarNaviChildVC:[[BaseNavigationController alloc]init]];
        }
    }
    return controller;
}
...
- (void)addChildVc:(NSArray *)childVcs titles:(NSArray *)titles images:(NSArray *)images selectedImages:(NSArray *)selectedImages tabBarNaviChildVC:(BaseNavigationController *)tabBarNaviChildVC{
