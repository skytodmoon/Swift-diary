...
@implementation QJLStageView

+ (instancetype)stageViewWithStage:(QJLStage *)stage frame:(CGRect)frame {
    QJLStageView *stageView = [QJLStageView viewFromNib];
...
- (void)startUnLockAnmiation {
    [[QJLSoundToolManager sharedSoundToolManager] playSoundWithSoundName:kSoundUnlock];
    CAAnimationGroup *group = [self shakeAndScaleAnimation];
    group.delegate = self;
    [self.layer addAnimation:group forKey:nil];
    
    self.stage.userInfo.unlock = YES;
    [[QJLStageInfoManager sharedStageInfoManager] saveStageInfo:self.stage.userInfo];
}
...
- (void)animationDidStop:(CAAnimation *)anim finished:(BOOL)flag {
    ...
    [[QJLSoundToolManager sharedSoundToolManager] playSoundWithSoundName:kSoundNewShow];
