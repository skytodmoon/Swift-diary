...
@interface QJLStage22PeopleView ()
{
    int _clickCount;
    BOOL _isReomve;
}
...
@property (nonatomic, strong) QJLStrokeLabel *countLabel;
...
@implementation QJLStage22PeopleView
...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:CGRectMake(0, 0, ScreenWidth, ScreenHeight - ScreenWidth / 3)]) {
        ...
        self.countLabel = [[QJLStrokeLabel alloc] initWithFrame:CGRectMake(0, 0, ScreenWidth / 3, 50)];
...
- (void)startFart {
    ...
    for (int fartCount = 1; fartCount <= _count; fartCount++) {
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)((fartCount - 1) * 0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            ...
            [[QJLSoundToolManager sharedSoundToolManager] playSoundWithSoundName:soundName];
