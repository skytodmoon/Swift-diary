...
@property (nonatomic, assign) QJLSubjectGuessType guessType;
@property (nonatomic, assign) QJLSubjectGuessType coverType;


@end

@implementation QJLSubjectView
...
- (void)showNextSubjectViewNums:(void (^)(int, int, int, int))nums {
    ...
    BOOL isMoveLeft = !(self.coverType == QJLSubjectGuessTypeLeft);
    BOOL isMoveMiddle = !(self.coverType == QJLSubjectGuessTypeMiddle);
    BOOL isMoveRight = !(self.coverType == QJLSubjectGuessTypeRight);
    ...
    [UIView animateWithDuration:duration animations:^{
        ...
        if (!isMoveLeft) {
            ...
            if (self.guessType == QJLSubjectGuessTypeMiddle) {
                _rightRandom = arc4random_uniform(10);
                _middleRandom = -1;
                while (_rightRandom - _leftRandom < 0) {
                    _rightRandom = arc4random_uniform(10);
                }
            } else if (self.guessType == QJLSubjectGuessTypeRight) {
            ...
            while (self.guessType == QJLSubjectGuessTypeMiddle) {
                self.guessType = arc4random_uniform(3);
            }
            
            if (self.guessType == QJLSubjectGuessTypeLeft) {
                _rightRandom = arc4random_uniform(10);
                _leftRandom = -1;
        ...
        } else if (self.guessType == QJLSubjectGuessTypeMiddle) {
...
- (void)showResultWithResult:(int)result finish:(void (^)(void))finish {
    ...
    switch (self.guessType) {
        case QJLSubjectGuessTypeLeft:
            self.leftIV.image = resultImage;
            break;
        case QJLSubjectGuessTypeMiddle:
            self.middleIV.image = resultImage;
            break;
        case QJLSubjectGuessTypeRight:
...
- (void)calculationHandImageViewToCenter {
    ...
    if (self.coverType == QJLSubjectGuessTypeLeft) {
        _handToCenterY = self.leftIV.center.y;
        _handToCenterX = self.leftIV.center.x;
    } else if (self.coverType == QJLSubjectGuessTypeMiddle) {
