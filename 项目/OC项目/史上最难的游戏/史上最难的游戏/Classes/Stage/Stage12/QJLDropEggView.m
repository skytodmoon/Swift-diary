...
- (NSInteger)showHandAndResult {
    NSInteger scroe;
    QJLResultStateType stateType;
    
    [[QJLSoundToolManager sharedSoundToolManager] playSoundWithSoundName:kSoundEggYearName];
    ...
    if (toBottomMargin < 20) {
        stateType = QJLResultStateTypePerfect;
    } else if (toBottomMargin < 50) {
        stateType = QJLResultStateTypeGreat;
    } else if (toBottomMargin < 100) {
        stateType = QJLResultStateTypeGood;
    } else {
        stateType = QJLResultStateTypeOK;
    }
    ...
    QJLState12ResultView *resultView = [[QJLState12ResultView alloc] initWithFrame:CGRectMake(0, CGRectGetMinY(whiteView.frame) - 150, ScreenWidth / 3, 120)];
    resultView.alpha = 0;
