...
@implementation QJLStage13GuessView
...
- (BOOL)guessPeopleWithGuessType:(QJLStage13GuessType)type {
    ...
    if (type == QJLStage13GuessTypeMan) {
        result = _hasMan;
        _leftSucess = YES;
    }
    
    if (type == QJLStage13GuessTypeChild) {
        result = _hasChild;
        _middleSucess = YES;
    }
    
    if (type == QJLStage13GuessTypeOld) {
        result = _hasOld;
        _rightSucess = YES;
    }
...
- (void)stopAnimationWithTimeOver {
    ...
    [[QJLSoundToolManager sharedSoundToolManager] playSoundWithSoundName:kSoundMoanName(arc4random_uniform(3) + 2)];
