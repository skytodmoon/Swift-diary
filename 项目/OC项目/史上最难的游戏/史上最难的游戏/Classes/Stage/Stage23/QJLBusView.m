...
typedef NS_ENUM(NSInteger, QJLDirectionType) {
    QJLDirectionTypeLeft = 0,
    QJLDirectionTypeRight
};

@interface QJLBusView ()
{
    int _count;
    float _speed;
...
@property (nonatomic, assign) QJLDirectionType direction;



@end

@implementation QJLBusView
...
- (void)showBus {
    _count++;
    [[QJLSoundToolManager sharedSoundToolManager] playSoundWithSoundName:kSoundBusPassName];
    _redBoyNum = 0;
    _yellowGirlNum = 0;
    _blueBoyNum = 0;
    ...
    if (_count % 2) {
        ...
        self.direction = QJLDirectionTypeLeft;
    } else {
        self.transform = CGAffineTransformScale(self.transform, -1, 1);
        self.direction = QJLDirectionTypeRight;
    }
...
- (void)updateTime {
    ...
    if ((self.transform.tx <= -self.frame.size.width * 2 && self.direction == QJLDirectionTypeLeft) || (self.transform.tx >= 0 && self.direction == QJLDirectionTypeRight)) {
