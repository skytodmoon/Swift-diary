...
- (void)setUp
{
    ...
    if (_isShowTitleGradient && _titleColorGradientStyle == TitleColorGradientStyleRGB) {
...
// 一次性设置所有颜色渐变属性
- (void)setUpTitleGradient:(void (^)(BOOL *, TitleColorGradientStyle *, CGFloat *, CGFloat *, CGFloat *, CGFloat *, CGFloat *, CGFloat *))titleGradientBlock
{
...
- (void)viewDidLayoutSubviews
{
    ...
    CGFloat contentY = self.navigationController?NavBarH : [UIApplication sharedApplication].statusBarFrame.size.height;
    CGFloat contentW = ScreenW;
    CGFloat contentH = self.tabBarController ? ScreenH - contentY - self.tabBarController.tabBar.height :  ScreenH - contentY;
    // 设置整个内容的尺寸
    ...
    // 设置标题滚动视图frame
    // 计算尺寸
    CGFloat titleH = _titleHeight?_titleHeight:TitleScrollViewH;
    ...
    self.contentScrollView.frame = _isfullScreen?CGRectMake(0, 0, contentW, ScreenH) :CGRectMake(0, contentScrollY, contentW, self.contentView.height - contentScrollY);
...
- (void)selectLabel:(UILabel *)label
{
    ...
    // 标题缩放
    if (_isShowTitleScale && _titleColorGradientStyle == TitleColorGradientStyleRGB) {
