...
//  Copyright © 2017年 . 醉看红尘这场梦All rights reserved.
//
...
class AccountModel: NSObject {
    ...
    class func checkUserInfo() {
        if isLogin() {
            ...
            NetworkTool.shareNetworkTool.post(GET_USERINFO, parameters: parameters, finished: { (success, result, error) in
                if success {
                    print("登录信息有效")
    ...
    class func logout() -> Void {
        ...
        do {
            try NSFileManager.defaultManager().removeItemAtPath(AccountModel.accountPath)
        } catch {
            print("退出异常")
        }
    }
    ...
    // 保存到内存中
    private static var userAccount: AccountModel?
    
    static func shareAccount() -> AccountModel? {
        if userAccount == nil {
            userAccount = NSKeyedUnarchiver.unarchiveObjectWithFile(accountPath) as? AccountModel
        }
        if userAccount == nil {
            // 说明没有登录
            return nil
