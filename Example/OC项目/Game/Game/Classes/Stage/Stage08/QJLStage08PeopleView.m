...
@interface QJLStage08PeopleView ()
{
    BOOL _isModel1;
    BOOL _isModel2;
    BOOL _isModel3;
...
@implementation QJLStage08PeopleView
...
- (void)showCurtain {
    [[QJLSoundToolManager sharedSoundToolManager] playSoundWithSoundName:kSoundSayOKName];
...
- (BOOL)takePhotoWithIndex:(int)index {
    BOOL isSucess;
    [[QJLSoundToolManager sharedSoundToolManager] playSoundWithSoundName:kSoundTakePhotoName];
    switch (index) {
        case 0:
            if (_isModel1) {
                isSucess = YES;
