...
@property (weak, nonatomic) IBOutlet QJLResultScoreView *scroeView;
@property (weak, nonatomic) IBOutlet UIImageView *blurBackIV;
@property (nonatomic, strong) QJLHighScroeTextView *highScroeView;
...
@implementation QJLResultViewController
...
- (void)viewDidLoad {
    ...
    self.highScroeView = [[QJLHighScroeTextView alloc] initWithFrame:CGRectMake(0, CGRectGetMinY(self.scroeImageView.frame) - 100, ScreenWidth, 200)];
    ...
    if (sender.tag == 20) {
        for (UIViewController *vc in self.navigationController.viewControllers) {
            if ([vc isKindOfClass:[QJLPrepareViewController class]]) {
                _stage.userInfo = [[QJLStageInfoManager sharedStageInfoManager] stageInfoWithNumber:_stage.num];
                ((QJLPrepareViewController *)vc).stage = _stage;
...
#pragma mark QJLResultScoreViewDelegate
- (void)resultScoreViewChangeWithRank:(NSString *)rank {
    ...
    [[QJLSoundToolManager sharedSoundToolManager] playSoundWithSoundName:[NSString stringWithFormat:@"scoreGrade_%@.mp3", rank]];
}
...
- (void)resultScoreViewShowNewCount {
    ...
    [UIView animateWithDuration:0.3 animations:^{
        ...
        [self shakeAnimation];
        [[QJLSoundToolManager sharedSoundToolManager] playSoundWithSoundName:kSoundNewRecordName1];
        [[QJLSoundToolManager sharedSoundToolManager] playSoundWithSoundName:kSoundNewRecordName2];
...
- (void)resultScoreViewShowFailViewPassScroeStr:(NSString *)passScroe userScroeStr:(NSString *)userScroe {
    ...
    [[QJLSoundToolManager sharedSoundToolManager] playSoundWithSoundName:kSoundFailDropName];
    ...
    [UIView animateWithDuration:0.2 delay:0.1 options:UIViewAnimationOptionCurveLinear animations:^{
        ...
        [[QJLSoundToolManager sharedSoundToolManager] playSoundWithSoundName:kSoundFailScreamName];
    ...
    [UIView animateWithDuration:0.15 delay:0 options:UIViewAnimationOptionCurveLinear animations:^{
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.15 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            ...
            [UIView animateWithDuration:0.3 delay:0 options:UIViewAnimationOptionCurveLinear animations:^{
                ...
                [[QJLSoundToolManager sharedSoundToolManager] playSoundWithSoundName:kSoundcageDropName];
            }];
        });
    }];
}
