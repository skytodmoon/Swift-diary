...
@implementation QJLResultScoreView

- (void)awakeFromNib {
    _stageInfo = [[QJLStageInfo alloc] init];
    for (int i = 0; i < 7; i++) {
...
- (void)showResult {
    ...
    if ([_stageInfo.rank isEqual:@"s"]) {
        [[QJLSoundToolManager sharedSoundToolManager] playSoundWithSoundName:kSoundSName];
    }
    
    [[QJLSoundToolManager sharedSoundToolManager] playSoundWithSoundName:kSoundScroeNormalName];
...
- (BOOL)isNewCount {
    if (_stage.max - _stage.min > 0) {
        if (_newScroe > [[QJLStageInfoManager sharedStageInfoManager] stageInfoWithNumber:_stageInfo.num].score) {
            return YES;
        }
    } else {
        if (_newScroe < [[QJLStageInfoManager sharedStageInfoManager] stageInfoWithNumber:_stageInfo.num].score) {
...
- (void)saveStageUserInfo {
    QJLStageInfo *lastStageInfo = [[QJLStageInfoManager sharedStageInfoManager] stageInfoWithNumber:_stageInfo.num];
    
    if (!lastStageInfo || !lastStageInfo.rank || lastStageInfo.score == 0) {
        [[QJLStageInfoManager sharedStageInfoManager] saveStageInfo:_stageInfo];
        return;
    }
