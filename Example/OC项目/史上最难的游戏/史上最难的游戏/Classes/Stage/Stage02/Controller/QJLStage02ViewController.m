...
#import "QJLGuessFingerView.h"
#import "PrefixHeader.pch"
#define kImageViewHeight 181
#define kImageViewWidth  40

@interface QJLStage02ViewController ()
{
    int _count;
    BOOL _stop;
    int _scroe;
...
@property (nonatomic, strong) QJLGuessFingerView *guessView;
...
@implementation QJLStage02ViewController
...
- (void)buildGuessImageView {
    self.guessView = [[QJLGuessFingerView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(self.countScore.frame) + 40, ScreenWidth, 150)];
    [self.view insertSubview:self.guessView aboveSubview:self.winImageView];
    
    __weak typeof(self) weakSelf = self;
    self.guessView.animationFinish = ^(int winIndex){
        [[QJLSoundToolManager sharedSoundToolManager] playSoundWithSoundName:kSoundAppearSoundName];
...
- (void)calculateScroeWithSecond:(int)second ms:(int)ms isRight:(BOOL)right {
    ...
    if (right) {
        [[QJLSoundToolManager sharedSoundToolManager] playSoundWithSoundName:kSoundRightSoundName];
        if (second == 0) {
            if (ms < 20) {
        ...
        [[QJLSoundToolManager sharedSoundToolManager] playSoundWithSoundName:kSoundWrongSoundName];
