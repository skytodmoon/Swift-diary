...
@interface HomeCourseDetailInfoCell ()
{
...
@implementation HomeCourseDetailInfoCell
...
-(void)setHomeCourseDM:(HomeCourseDetailModel *)HomeCourseDM{
    _jzCourseDM = HomeCourseDM;
    [_imageView sd_setImageWithURL:[NSURL URLWithString:HomeCourseDM.PhotoURL] placeholderImage:[UIImage imageNamed:@"lesson_default"]];
    
    _courseNameLabel.text = HomeCourseDM.CourseName;
    _numsLabel.text = [NSString stringWithFormat:@"%@人",HomeCourseDM.StudentNumber];
    _schoolNameLabel.text = [NSString stringWithFormat:@"  %@",HomeCourseDM.SchoolName];
    if ([HomeCourseDM.Cost isEqualToString:@"0"]) {
        _priceLabel.text = @"￥免费";
    }else{
        int cost = [HomeCourseDM.Cost intValue];
