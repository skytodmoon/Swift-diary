...
extension MeViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(tableView: UITableView, didSelectRowAtIndexPath indexPath: NSIndexPath) {
        if indexPath.section == 0 {
             ...
             *  留言反馈
             */
            if indexPath.row == SDMineCellType.Feedback.hashValue {
                let feedbackVC = FeedbackViewController()
                navigationController?.pushViewController(feedbackVC, animated: true)
                /**
                 *  个人中心
                 */
            }else if indexPath.row == SDMineCellType.MyCenter.hashValue {
                if UserAccountTool.userIsLogin() {
                 ...
                 *   我的收藏
                 */
            } else if indexPath.row == SDMineCellType.MyCollect.hashValue {
                /**
                 *   我的订单
                 */
            } else if indexPath.row == SDMineCellType.MyOrder.hashValue {
                if UserAccountTool.userIsLogin() {
