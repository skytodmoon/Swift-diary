...
@interface QJLStage16ViewController ()
{
    BOOL _frist;
}

@property (nonatomic, strong) QJLStage16PeopleView *peopleView;
@property (nonatomic, strong) QJLCountDownLabel *timeLabel;
@property (nonatomic, assign) int count;

@end

@implementation QJLStage16ViewController
...
- (void)buildStageInfo {
    ...
    self.peopleView = [QJLStage16PeopleView viewFromNib];
    [self.view insertSubview:self.peopleView belowSubview:bottomBlackView];
    
    self.timeLabel = [[QJLCountDownLabel alloc] initWithFrame:CGRectMake(210, 325, 110, 90) startTime:6.0 textSize:60];
...
- (void)btnClick:(UIButton *)sender {
    ...
    if (_frist) {
        ...
        [(QJLCountDownLabel *)self.timeLabel startCountDownWithCompletion:^{
...
- (void)pauseGame {
    [(QJLCountDownLabel *)self.timeLabel pause];
...
- (void)continueGame {
    [super continueGame];
    [(QJLCountDownLabel *)self.timeLabel continueWork];
}
