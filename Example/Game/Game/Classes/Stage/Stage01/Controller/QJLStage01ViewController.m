...
#import "QJLFootView.h"
#import "QJLFeatherView.h"
...
@interface QJLStage01ViewController ()

@property (nonatomic, strong) QJLCountDownLabel *timeLabel;
@property (nonatomic, strong) QJLFootView *footView;
@property (nonatomic, strong) QJLFeatherView *featherView;

@end
 
@implementation QJLStage01ViewController
...
- (void)initTimeLabel {
    self.timeLabel = [[QJLCountDownLabel alloc] initWithFrame:CGRectMake(ScreenWidth - 55, ScreenHeight - self.redButton.frame.size.height - 50, 60, 50)
                                                    startTime:kStage01Duration textSize:30];
    [self.view insertSubview:self.timeLabel aboveSubview:self.redButton];
}

- (void)initFootView {
    self.footView = [[QJLFootView alloc] initWithFrame:CGRectMake(0, ScreenHeight - self.redButton.frame.size.height - 200 - 45, ScreenWidth / 3, 200)];
    [self.view insertSubview:self.footView aboveSubview:self.redButton];
}

- (void)initFeaterView {
    self.featherView = [[QJLFeatherView alloc] initWithFrame:CGRectMake((ScreenWidth / 3 - 100) * 0.5, ScreenHeight - self.redButton.frame.size.height - 160, 100, 73)];
...
- (void)endGame {
    ...
    [self showResultControllerWithNewScroe:[((QJLScoreboardCountView *)self.countScore).countLabel.text intValue] unit:@"PTS" stage:self.stage isAddScore:YES];
}
