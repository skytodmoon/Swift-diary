//
//  ProfileFeedbackViewController.swift
//  爆侃网文
//
...
class ProfileFeedbackViewController: BaseTableViewController {
    ...
    @objc private func didTappedCommitButton(commitButton: UIButton) {
        ...
        ProgressHUD.showWithStatus("正在提交")
        ...
        NetworkTool.shareNetworkTool.post("http://120.24.79.174/jiansan/feedback.php", parameters: parameters) { (success, result, error) in
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW,Int64(2 * Double(NSEC_PER_SEC))), dispatch_get_main_queue()) {
                
                self.tableView.userInteractionEnabled = true
                if success == true {
                    ProgressHUD.showSuccessWithStatus("谢谢支持")
                } else {
                    ProgressHUD.showErrorWithStatus("您的网络不给力哦")
                }
