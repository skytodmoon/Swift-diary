...
class NewsDetailViewController: UIViewController {
    ...
    /// 赞分享cell
    private lazy var starAndShareCell: StarAndShareCell = {
        let starAndShareCell = self.tableView.dequeueReusableCellWithIdentifier(self.detailStarAndShareIdentifier) as! StarAndShareCell
    ...
    override func viewDidLoad() {
        ...
        tableView.registerNib(UINib(nibName: "StarAndShareCell", bundle: nil), forCellReuseIdentifier: detailStarAndShareIdentifier)
        tableView.registerClass(UITableViewCell.classForCoder(), forCellReuseIdentifier: detailOtherLinkIdentifier)
        tableView.registerNib(UINib(nibName: "CommentCell", bundle: nil), forCellReuseIdentifier: detailCommentIdentifier)
    ...
    func didTappedmoreCommentButton(button: UIButton) -> Void {
        let commentVc = CommentTableViewController(style: UITableViewStyle.Plain)
                ...
                if let successResult = result {
                    ...
                    if let others = otherLinks {
                        for other in others {
                            ...
                            let otherModel = OtherLinkModel(dict: dict)
                    ...
                    self.model = ArticleDetailModel(dict: dict)
                }
            } else {
                print("error:\(error)")
            }
    ...
    func loadCommentList(classid: String, id: String) {
        ...
        NetworkTool.shareNetworkTool.get(GET_COMMENT, parameters: parameters as? [String : AnyObject]) { (success, result, error) -> () in
            ...
            if success {
                if let successResult = result {
                    ...
                    for comment in data.reverse() {
                        ...
                        let commentModel = CommentModel(dict: dict as! [String : AnyObject])
                ...
                ProgressHUD.showErrorWithStatus("网络不给力")
            }
        }
    }
    ...
    /// 底部条
    private lazy var bottomBarView: NewsBottomBar = {
        let bottomBarView = NSBundle.mainBundle().loadNibNamed("NewsBottomBar", owner: nil, options: nil).last as! NewsBottomBar
...
extension NewsDetailViewController: UITableViewDataSource, UITableViewDelegate {
     ...
     */
    func loadWebViewContent(model: ArticleDetailModel) {
    ...
    func tableView(tableView: UITableView, heightForRowAtIndexPath indexPath: NSIndexPath) -> CGFloat {
        ...
        case 4:
            ...
            if rowHeight < 1 {
                let cell = tableView.dequeueReusableCellWithIdentifier(detailCommentIdentifier) as! CommentCell
...
extension NewsDetailViewController: NewsBottomBarDelegate, CommentCommitViewDelegate, SetFontViewDelegate {
    ...
    func didTappedCollectButton(button: UIButton) {
        ...
        if AccountModel.isLogin() {
            ...
            NetworkTool.shareNetworkTool.post(ADD_DEL_FAVA, parameters: parameters) { (success, result, error) in
                if success {
                    if let successResult = result {
                        if successResult["result"]["status"].intValue == 1 {
                            // 增加成功
                            ProgressHUD.showSuccessWithStatus("收藏成功")
                            button.selected = true
                        } else if successResult["result"]["status"].intValue == 3 {
                            // 删除成功
                            ProgressHUD.showSuccessWithStatus("取消收藏")
    ...
    func didTappedSendButtonWithMessage(message: String) {
        ...
        NetworkTool.shareNetworkTool.get(SUBMIT_COMMENT, parameters: parameters) { (success, result, error) in
            if success {
                // 加载数据
                self.updateData()
            }
...
extension NewsDetailViewController: CommentCellDelegate {
    func didTappedStarButton(button: UIButton, commentModel: CommentModel) {
        ...
        NetworkTool.shareNetworkTool.get(TOP_DOWN, parameters: parameters as? [String : AnyObject]) { (success, result, error) in
            print(result)
